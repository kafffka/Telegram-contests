...
public class MrzRecognizer {
	...
	private static Result recognizeBarcode(Bitmap bitmap) {
		BarcodeScannerOptions barcodeScannerOptions = new BarcodeScannerOptions.Builder().build();
		...
		try {
			List<Barcode> barcodes = Tasks.await(barcodeScanner.process(InputImage.fromBitmap(bitmap, 0)));
			for (int i = 0; i < barcodes.size(); i++) {
				Barcode code = barcodes.get(i);
				if (code.getValueType() == Barcode.TYPE_DRIVER_LICENSE && code.getDriverLicense() != null) { // BarcodeDetector has built-in support for North American driver licenses/IDs
					Result res = new Result();
					res.type = "ID".equals(code.getDriverLicense().getDocumentType()) ? Result.TYPE_ID : Result.TYPE_DRIVER_LICENSE;
					switch (code.getDriverLicense().getIssuingCountry()) {
						case "USA":
					...
					res.firstName = capitalize(code.getDriverLicense().getFirstName());
					res.lastName = capitalize(code.getDriverLicense().getLastName());
					res.middleName = capitalize(code.getDriverLicense().getMiddleName());
					res.number = code.getDriverLicense().getLicenseNumber();
					if (code.getDriverLicense().getGender() != null) {
						switch (code.getDriverLicense().getGender()) {
							case "1":
					...
					try {
						if (code.getDriverLicense().getBirthDate() != null && code.getDriverLicense().getBirthDate().length() == 8) {
							res.birthYear = Integer.parseInt(code.getDriverLicense().getBirthDate().substring(yearOffset, yearOffset + 4));
							res.birthMonth = Integer.parseInt(code.getDriverLicense().getBirthDate().substring(monthOffset, monthOffset + 2));
							res.birthDay = Integer.parseInt(code.getDriverLicense().getBirthDate().substring(dayOffset, dayOffset + 2));
						}
						if (code.getDriverLicense().getExpiryDate() != null && code.getDriverLicense().getExpiryDate().length() == 8) {
							res.expiryYear = Integer.parseInt(code.getDriverLicense().getExpiryDate().substring(yearOffset, yearOffset + 4));
							res.expiryMonth = Integer.parseInt(code.getDriverLicense().getExpiryDate().substring(monthOffset, monthOffset + 2));
							res.expiryDay = Integer.parseInt(code.getDriverLicense().getExpiryDate().substring(dayOffset, dayOffset + 2));
						}
					} catch (NumberFormatException ignore) {
					}

					return res;
				} else if (code.getValueType() == Barcode.TYPE_TEXT && code.getFormat() == Barcode.FORMAT_PDF417) { // Russian driver licenses (new-ish ones) use a non-very-much-documented format
					...
					if (code.getRawValue().matches("^[A-Za-z0-9=]+$")) {
						try {
							byte[] _data = Base64.decode(code.getRawValue(), 0);
